...
>  ВАЖНО: значения некоторых параметров являются динамическими — они меняются при каждом рендеринге страницы с Yidun.  
> Извлекайте их непосредственно перед созданием задачи, чтобы избежать ошибок при решении.
> Примеры извлечения параметров см. в разделе [Как найти все нужные параметры для создания задачи](#как-найти-все-нужные-параметры-для-создания-задачи).
</span>
<br />
...
<TabItem value="proxy" label="YidunTask (при использовании прокси)" className="bordered-panel">
  ...
  **Важно**: должен отличаться от стандартных `c.dun.163.com` и `c-v6.dun.163.com`.  
  Указывайте только домен без префикса `https://`  
