...
### Как получить websiteKey
Откройте *Инструменты разработчика*, активируйте капчу, перейдите во вкладку **Network** и среди запросов найдите тот, который начинается на `get?referer=`. Значение *id* означает `websiteKey`.
