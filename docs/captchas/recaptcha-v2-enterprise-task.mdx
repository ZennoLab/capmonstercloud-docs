...
Используйте метод [getTaskResult](../api/methods/get-task-result.mdx) чтобы получить решение ReCaptcha2. В зависимости от загрузки системы вы получите ответ через время в диапазоне от 10 с до 80 с.
  ...
  <details>
    ...
    document.addEventListener('DOMContentLoaded', async () => {
      ...
      let recaptchaV2EnterpriseRequest = new RecaptchaV2EnterpriseRequest({
        ...
        enterprisePayload: {
          s: "SOME_ADDITIONAL_TOKEN",   // Опциональный enterprise параметр
        }
      });
