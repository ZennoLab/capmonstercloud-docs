...
:::warning **Внимание!**
CapMonster Cloud по умолчанию использует встроенные прокси, которые подходят для большинства сайтов.
**Если токен не принимается сервером**, необходимо повторить попытку с собственными прокси — это повышает вероятность успешной проверки.

Рекомендуемый порядок действий:

- Сначала выполните решение капчи без прокси.

- Если токен отклонён целевым сайтом — повторите с использованием своих прокси.
