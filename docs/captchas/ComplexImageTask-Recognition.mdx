...
### Создание задачи `dli_ensemble`

В запросе нужно передать одно изображение (вместе с текстом) в формате base64.
...
### Получение результата задачи `dli_ensemble`
...
complexImage_request = RecognitionComplexImageTaskRequest(
    imagesBase64=[
        "/9xwee/"  # изображение в формате base64
    ],
    metadata={"Task": "oocl_rotate"}
)
...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        var recognitionCITRequest = new RecognitionComplexImageTaskRequest
        {
            ...
            {
                "/9j/4AAQSkZJRgABAQEAAAAAAAD" // Изображение в формате base64
            },
