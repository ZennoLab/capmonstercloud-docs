...
<BlogLink url="https://blog.capmonster.cloud/ru/blog/news/imperva-incapsula-news"/>
...
<TabItem value="proxyless" label="CustomTask (без прокси)" default className="bordered-panel">
    ...
    **Imperva**
	...
	Содержимое скрипта, загруженное этим src, должно быть закодировано в формате base64. Это и будет значение для параметра incapsulaScriptBase64.
	...
	**Передавайте только актуальный UA. Сейчас таковым является**: userAgentPlaceholder
</TabItem>
...
<Tabs className="full-width-tabs filled-tabs request-tabs" groupId="captcha-type">
  <TabItem value="js" label="JavaScript" default className="method-panel">
    ...
    import { CapMonsterCloudClientFactory, ClientOptions, ImpervaCustomTaskProxylessRequest } from '@zennolab_com/capmonstercloud-client';

    document.addEventListener('DOMContentLoaded', async () => {
      const cmcClient = CapMonsterCloudClientFactory.Create(new ClientOptions({ clientKey: '<your capmonster.cloud API key>' }));
      console.log(await cmcClient.getBalance());

      const imperva_request = new ImpervaCustomTaskProxylessRequest({
      ...
      console.log(await cmcClient.Solve(imperva_request));
    });
    ```
  </TabItem>
  ...
  <TabItem value="csharp" label="C#" className="method-panel">
    ...
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            var imperva_request = new ImpervaCustomTaskProxylessRequest
            {
                websiteUrl = "https://example.com", // URL with captcha
                userAgent = "userAgentPlaceholder",
				metadata = new ImpervaCustomTaskProxylessRequest.ImpervaMetadata
				{
					incapsulaScriptBase64 = "dmFyIF8weGQ2ZmU9Wydce..eDUzXHg2YVx4NGYnKV09XzB4Mjk3MTIxO319KCkpOw==",
					incapsulaSessionCookie = "l/LsGnrvyB9lNhXI8borDKa2IGcAAAAAX0qAEHheCWuNDquzwb44cw=",
					reese84UrlEndpoint = "Built-with-the-For-hopence-Hurleysurfecting-the-"
            ...
            var impervaResult = await cmCloudClient.SolveAsync(imperva_request);

            Console.WriteLine("Captcha Solution: " + string.Join(", ", impervaResult.Solution.Data));
            Console.WriteLine("Captcha Solution: " + string.Join(", ", impervaResult.Solution.Headers));
        }
    }
    ```
  </TabItem>
