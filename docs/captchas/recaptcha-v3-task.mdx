...
<TabItem value="proxyless" label="RecaptchaV3TaskProxyless (без прокси)" default className="bordered-panel">
   ...
   Укажите `true`, если необходимо решить reCAPTCHA в версии Enterprise — в этом случае задача будет обрабатываться так же, как [RecaptchaV3EnterpriseTask](https://docs.capmonster.cloud/ru/docs/captchas/recaptcha-v3-enterprise-task/).
