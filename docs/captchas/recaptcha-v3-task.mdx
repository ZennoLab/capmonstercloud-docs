...
<TabItem value="proxyless" label="RecaptchaV3TaskProxyless (без прокси)" default className="bordered-panel">
   ...
   Версия reCAPTCHA. Поддерживаемые значения: <code>true</code> / <code>false</code> (также допускается <code>1</code> / <code>0</code>, тип integer).

  Укажите <code>true</code>, если необходимо решение reCAPTCHA в версии Enterprise: в таком случае задача будет обрабатываться так же, как <a href="https://docs.capmonster.cloud/ru/docs/captchas/recaptcha-v3-enterprise-task/" target="_blank">RecaptchaV3EnterpriseTask</a>.
