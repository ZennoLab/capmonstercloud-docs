...
>  ВАЖНО: значения некоторых параметров являются динамическими — они меняются при каждом рендеринге страницы с Altcha.  
...
> **Важно:** параметр `iterations` соответствует значению `maxnumber`!
  ...
  > **Важно:** отправляйте значение поля `salt` целиком!
Если сайт присылает, например, такое значение:
...
Используйте метод [getTaskResult](../api/methods/get-task-result.mdx), чтобы получить решение Altcha.
