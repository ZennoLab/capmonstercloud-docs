...
Если сервер возвращает cookies в следующем формате (четыре строки):
...
Если cookies не возвращаются или возвращаются в одном из следующих форматов (обычно три строки):
...
2. **Получение cookies.** Из ответа извлекаются cookies защиты. Если возвращается cookie `TSPD_101_DID` (означающие, что защита уже пройдена) или формат неправильный — выполнение прекращается.
...
5. **Финальный запрос.** Запрос повторяется с полученными cookies, после чего возвращается нормальная HTML-страница.
...
const API_KEY = "YOUR_API_KEY"; // Замените на ваш API-ключ CapMonster
...
// Используйте актуальный User-Agent
const userAgent =
  "userAgentPlaceholder";
...
function extractValidTspdCookies(setCookies) {
  ...
  // Проверяем, что TSPD_101_DID отсутствует
  ...
  // Извлекаем только TS cookies
  const tsCookies = cookies.filter((c) => c.startsWith("TS"));

  // Должно быть только 4 TS cookies
  ...
  try {
    ...
    const response = await gotScraping(url, {
      ...
      headers: {
        ...
        referer: "https://example.com/", // Замените на актуальный referer
      },
    });
