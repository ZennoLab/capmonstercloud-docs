...
:::warning **Внимание!**
CapMonster Cloud по умолчанию работает через встроенные прокси — они уже включены в стоимость. Указывать собственные прокси требуется только в тех случаях, когда сайт не принимает токен или доступ к встроенным сервисам ограничен. Вы можете обратиться в [службу поддержки](https://helpdesk.zennolab.com/conversation/new), чтобы уточнить детали и узнать, насколько корректно принимаются токены на конкретном сайте.
...
<Tabs className="full-width-tabs filled-tabs request-tabs" groupId="captcha-type">
  <TabItem value="js" label="JavaScript" default className="method-panel">
    ...
    document.addEventListener('DOMContentLoaded', async () => {
      ...
      // Вариант 2. С прокси (раскомментируйте для использования)
      // const geetestV3Request = new GeeTestRequest({
  ...
  <TabItem value="python" label="Python" className="method-panel">
    ...
    # Вариант 2: С прокси (если сайт требует решение именно с вашего IP)
...
<Tabs className="full-width-tabs filled-tabs request-tabs" groupId="captcha-type">
  <TabItem value="js" label="JavaScript" default className="method-panel">
    ...
    document.addEventListener('DOMContentLoaded', async () => {
      const cmcClient = CapMonsterCloudClientFactory.Create(
        new ClientOptions({ clientKey: '<ваш API ключ CapMonster Cloud>' })
      );

      console.log(await cmcClient.getBalance());

      // Вариант 1: Решение GeeTest V4 без прокси
      ...
      // Вариант 2: Решение с прокси (раскомментируйте и заполните данные, если нужно)
      /*
      const geetestV4Request = new GeeTestRequest({
        websiteURL: 'https://example.com/geetest.php',
        gt: '81dc9bdb52d04dc20036dbd8313ed055',
  ...
  <TabItem value="python" label="Python" className="method-panel">
    ...
    geetest_request = GeetestRequest(
        websiteUrl="https://example.com/",  # URL страницы с капчей
        gt="54088bb07d2df3c46b79f80300b0abbe", # Замените на корректное значение
        version=4,
        initParameters={
            "riskType": "slide" 
        }
    ...
    async def solve_captcha():
        # Решаем капчу через CapMonster Cloud
        return await cap_monster_client.solve_captcha(geetest_request)
