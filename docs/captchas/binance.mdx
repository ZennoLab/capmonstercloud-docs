...
> Примеры извлечения параметров см. в разделе [Как получить websiteKey и validateId](#как-получить-websitekey-и-validateid).
</span>
<br />
  ...
  <TabItem value="proxy" label="BinanceTask (при использовании прокси)" className="bordered-panel">
    ...
    **BinanceTask**
    ...
    Динамический ключ. Значение параметра `validateId`, `securityId` или `securityCheckResponseValidateId`. Его можно взять из трафика (см. описание ниже).
