...
>  ВАЖНО: значения некоторых параметров являются динамическими — они меняются при каждом рендеринге страницы с FunCaptcha.  
> Извлекайте их непосредственно перед созданием задачи, чтобы избежать ошибок при решении.
> Примеры извлечения параметров см. в разделах [Поиск параметров FunCaptcha](#поиск-параметров-funcaptcha) и [Автоматическое решение FunCaptcha с data[blob]](#автоматическое-решение-funcaptcha-с-datablob).
</span>
<br />
...
**Важно:** Указывайте только домен без префикса `https://`
