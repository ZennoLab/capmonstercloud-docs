...
const NewsFilter = ({ handleChange}) => {
  ...
  return (
    <div className={styles.categoryBlock}>
      {categories.map((category) => <div className={`${styles.categoryItem} ${selected === category.id ? styles.categorySelected : ''}`}onClick={() => onChangeCategory(category.id)}>{category.title}</div>)}
    </div>
  );
};
