...
<TabItem value="proxyless" label="GeeTestTaskProxyless (sem proxy)" default className="bordered-panel">
    ...
    **GeeTestTaskProxyless**
    ...
    A chave de identificação GeeTest para o domínio. Valor estático, raramente atualizado.<br />Se v4, então este é o parâmetro clientId.
    ...
    Número da versão (o padrão é 3). Valores possíveis: 3, 4.
    ...
    Parâmetros adicionais para a versão 4, usados juntamente com “riskType” (tipo de captcha/características de sua verificação).
...
<ParamItem title="gt" required type="string" />
A chave identificadora GeeTest para o domínio. Valor estático, raramente atualizado.<br />Se for v4, este é o parâmetro clientId.

---

<ParamItem title="challenge" required="required only for V3" type="string" />
<p>
Uma chave dinâmica.<br />Cada vez que nossa API for chamada, precisamos obter um novo valor de chave. Se o captcha for carregado na página, o valor `challenge` não será mais válido, e você receberá o [erro](../api/api-errors.md) `ERROR_TOKEN_EXPIRED`. <br />
Você será cobrado por tarefas com erro `ERROR_TOKEN_EXPIRED`.
É necessário examinar as requisições e encontrar aquela em que este valor é retornado e, antes de cada criação de tarefa de reconhecimento, executar esta requisição e extrair o desafio dela.
</p>
