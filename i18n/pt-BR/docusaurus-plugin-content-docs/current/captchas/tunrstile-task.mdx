...
document.addEventListener('DOMContentLoaded', async () => {
  const cmcClient = CapMonsterCloudClientFactory.Create(new ClientOptions({ clientKey: '<sua chave de API capmonster.cloud>' }));
  ...
  const turnstileRequest = TurnstileRequest({
    ...
    pageData: 'pageDataAqui',
    data: 'dataAqui',
    htmlPageBase64: 'htmlPageBase64Aqui',
    userAgent: 'userAgentAqui',
  });
