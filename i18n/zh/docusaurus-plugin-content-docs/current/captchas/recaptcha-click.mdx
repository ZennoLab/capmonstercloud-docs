...
<TabItem value="proxyless" label="RecaptchaV2EnterpriseTaskProxyless (without proxy)" default className="bordered-panel">
    ...
    **recaptcha**
    ...
    <ParamItem title="Grid （在元数据中）" required type="string" />
    图片网格大小。
    可能的值: 4x4, 3x3, 1x1

    ---

    <ParamItem title="TaskDefinition （在元数据中）" required="required (如果 Task 未填写)" type="string" />
    ...
    <ParamItem title="Task （在元数据中）" required="required (if TaskDefinition is not filled)" type="string" />
...
**Grid （在元数据中）**：与图像网格大小相关的附加元数据。

**TaskDefinition （在元数据中）**：任务描述标识符/类型，例如：`/m/015qff` 表示“点击红绿灯”。

![](./images/recaptcha-click/taskdefinition.png)

**Task（在元数据中）**：与任务相关的附加元数据。
