...
> 重要：某些参数的值是动态的——它们会在包含 Temu 的页面每次渲染时发生变化。  
> 请在创建任务之前立即提取这些参数，以避免在求解过程中出现错误。  
> 有关自动提取参数的示例，请参阅[如何查找任务创建所需的所有参数](#如何查找任务创建所需的所有参数)。
</span>
<br />
...
## 获取任务结果方法
使用 [getTaskResult](../api/methods/get-task-result.mdx) 方法获取 Temu 解决方案。
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    let temuRequest = new TemuRequest({
        class: "Temu",
        websiteURL: "https://www.example.com/bgn_verification.html?verifyCode=",  // Temu 页面 URL
