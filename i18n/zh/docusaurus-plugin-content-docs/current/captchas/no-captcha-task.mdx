...
相比于普通的图像验证码，此类验证码可能需要更长时间来解决，但通过我们发送给您的 `g-recaptcha-response` 值，在解决您的 ReCaptcha2 后的下一个 60 秒内仍然有效，可以弥补这一问题。

<BlogLink url="https://capmonster.cloud/zh/blog/re-1/how-to-bypass-recaptcha-v2-using-capmonster-cloud"/>

:::warning **注意！**
CapMonster Cloud 默认使用内置代理，适用于大多数网站。
**如果服务器拒绝令牌**，则应使用您自己的代理重新尝试——这将提高验证成功的几率。

推荐操作顺序：

* 首先，在不使用代理的情况下完成验证码。

* 如果目标网站拒绝令牌，请使用您自己的代理重新尝试。
  ...
  <TabItem value="js" label="JavaScript" default className="method-panel">
    ...
    document.addEventListener('DOMContentLoaded', async () => {
      ...
      const cmcClient = CapMonsterCloudClientFactory.Create(
        new ClientOptions({ clientKey: '<你的 capmonster.cloud API 密钥>' })
      );

      console.log(await cmcClient.getBalance());

      const recaptchaV2Request = new RecaptchaV2Request({
        websiteURL: 'https://lessons.zennolab.com/captchas/recaptcha/v2_simple.php?level=high', // 带有验证码的页面 URL
      ...
      const recaptchaV2Request = new RecaptchaV2Request({
        websiteURL: 'https://lessons.zennolab.com/captchas/recaptcha/v2_simple.php?level=high',
        websiteKey: '6Lcg7CMUAAAAANphynKgn9YAgA4tQ2KI_iqRyTwd',
        proxyType: 'http',           // 代理类型: http, https, socks4, socks5
        proxyAddress: '8.8.8.8',     
        proxyPort: 8080,             
        proxyLogin: 'proxyLoginHere',
        proxyPassword: 'proxyPasswordHere',
    ...
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            */
            ...
            Console.WriteLine("验证码结果: " + recaptchaV2Result.Solution.Value);
        }
    }
    ```
