...
<TabItem value="proxyless" label="RecaptchaV3TaskProxyless (without proxy)" default className="bordered-panel">
  ...
  <ParamItem title="isEnterprise" type="boolean" />
    reCAPTCHA 版本。 支持的值：`true` / `false`（也可使用 `1` / `0`，整数类型）。
    
    如果需要以 Enterprise 方式解决 reCAPTCHA，请设置 `true` —— 在这种情况下，任务将按照与 [RecaptchaV3EnterpriseTask](https://docs.capmonster.cloud/ru/docs/captchas/recaptcha-v3-enterprise-task/) 相同的方式处理。
