  ...
  <TabItem value="js" label="JavaScript" default className="method-panel">
    ...
    document.addEventListener('DOMContentLoaded', async () => {
      ...
      const geetestV3Request = new GeeTestRequest({
        websiteURL: 'https://example.com/geetest.php', // 带验证码页面的 URL
        gt: '81dc9bdb52d04dc20036dbd8313ed055',       // 替换为实际值
  ...
  <TabItem value="csharp" label="C#" className="method-panel">
    ...
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            {
                WebsiteUrl = "https://example.com/demo/geetest", // 带验证码页面的 URL
                Gt = "your_gt_value",       // 替换为实际值                     
...
<Tabs className="full-width-tabs filled-tabs request-tabs" groupId="captcha-type">
  ...
  <TabItem value="python" label="Python" className="method-panel">
    ...
    geetest_request = GeetestRequest(
        websiteUrl="https://example.com/",  # 带验证码页面的 URL
        gt="54088bb07d2df3c46b79f80300b0abbe", # 替换为实际值
        version=4,
        initParameters={
            "riskType": "slide" 
        }
