...
:::warning **警告！**
默认情况下，CapMonster Cloud 使用内置代理，因此无需连接自己的代理——其费用已包含在价格中。  
仅在令牌无法在目标网站上使用或对内置服务的访问受限时，才使用您自己的代理。您可以联系 [支持](https://helpdesk.zennolab.com/conversation/new) 了解详情，并检查令牌在特定网站上的可用性。  

如果您的代理使用基于 IP 的授权，必须将地址 **116.203.55.208** 加入白名单。  
:::
