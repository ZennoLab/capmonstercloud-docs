...
<BlogLink url="https://blog.capmonster.cloud/zh/blog/news/imperva-incapsula-news"/>
...
<TabItem value="proxyless" label="CustomTask (without proxy)" default className="bordered-panel">
    ...
    **Imperva**
    ...
    `"incapsulaScriptBase64": "..."` - Incapsula JavaScript 脚本的 base64 编码内容。 要获取此值，您需要访问脚本页面： https://site.com/_Incapsula_Resource?SWJIYLWA=719...3&ns=1&cb=1...1
	
	![](./images/incapsula/incapsula1.png)
	
	此src加载的脚本内容必须以base64格式进行编码。 这将是参数 incapsulaScriptBase64 的值。

    ---

    <ParamItem title="metadata.incapsulaSessionCookie" required type="string" />
	您从 Incapsula 获取的 Cookie。您可以在页面上使用 "document.cookie" 或在请求头 Set-Cookie: "incap_sess_*=..." 中获取
