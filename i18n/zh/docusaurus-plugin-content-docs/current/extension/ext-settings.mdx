...
安装扩展程序时，如果通过安装包进行安装（可通过链接下载：[Chrome 版本](https://zenno.link/chrome-actual-build) 和 [Firefox 版本](https://zenno.link/firefox-actual-build)），可以设置扩展程序的初始参数。为此，需要先解压安装包，编辑 **defaultSettings.json** 文件，然后重新打包。
