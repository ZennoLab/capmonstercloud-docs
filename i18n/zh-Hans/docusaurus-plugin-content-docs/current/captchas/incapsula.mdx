...
<TabItem value="proxyless" label="CustomTask (without proxy)" default className="bordered-panel">
    ...
    **Incapsula**
    ...
    `"incapsulaScriptBase64": "..."` - Incapsula JavaScript 脚本的 base64 编码内容。 

    ---

    <ParamItem title="metadata.incapsulaSessionCookie" required type="string" />
	您从 Incapsula 获取的 Cookie。您可以在页面上使用 "document.cookie" 或在请求头 Set-Cookie: "incap_sess_*=..." 中获取（参见 /createTask 请求示例）。
