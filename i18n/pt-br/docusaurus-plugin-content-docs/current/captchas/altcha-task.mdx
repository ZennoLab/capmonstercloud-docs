...
>  IMPORTANTE: alguns valores de parâmetros são dinâmicos — eles mudam a cada renderização da página que contém Altcha.  
> Extraia-os imediatamente antes de criar a tarefa para evitar erros durante a resolução.  
> Veja exemplos de extração de parâmetros na seção [Como encontrar todos os parâmetros necessários para a criação da tarefa](#como-encontrar-todos-os-parâmetros-necessários-para-a-criação-da-tarefa).
</span>
<br />
...
<TabItem value="proxyless" label="CustomTask (sem proxy)" default className="bordered-panel">
  ...
  **altcha**
  ...
  > **Importante:** sempre envie o valor completo do campo `salt`!
> Se o site retornar algo como:
