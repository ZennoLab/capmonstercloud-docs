...
>  IMPORTANTE: alguns valores de parâmetros são dinâmicos — eles mudam a cada renderização da página que contém Yidun.  
> Extraia-os imediatamente antes de criar a tarefa para evitar erros durante a resolução.  
> Veja exemplos de extração de parâmetros na seção [Como encontrar todos os parâmetros necessários](#como-encontrar-todos-os-parâmetros-necessários).
</span>
<br />
...
<TabItem value="proxy" label="YidunTask (com proxy)" className="bordered-panel">
  ...
  **Importante**: deve ser diferente dos padrões `c.dun.163.com` e `c-v6.dun.163.com`.  
  Indique apenas o domínio, sem o prefixo `https://`.
...
## Como encontrar todos os parâmetros necessários

`websiteURL` e `websiteKey`
