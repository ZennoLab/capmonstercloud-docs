...
<BlogLink url="https://blog.capmonster.cloud/pt-BR/blog/scraping/amazon-captcha-web-scraping"/>
...
<Tabs className="full-width-tabs filled-tabs request-tabs" groupId="captcha-type">
  <TabItem value="js" label="JavaScript" default className="method-panel">
    ...
    import { CapMonsterCloudClientFactory, ClientOptions, AmazonWafProxylessRequest } from '@zennolab_com/capmonstercloud-client';

    document.addEventListener('DOMContentLoaded', async () => {
      const cmcClient = CapMonsterCloudClientFactory.Create(new ClientOptions({ clientKey: '<your capmonster.cloud API key>' }));
      console.log(await cmcClient.getBalance());

      const amazonWafRequest = new AmazonWafProxylessRequest({
      ...
      console.log(await cmcClient.Solve(amazonWafRequest));
    });
    ```
  </TabItem>
