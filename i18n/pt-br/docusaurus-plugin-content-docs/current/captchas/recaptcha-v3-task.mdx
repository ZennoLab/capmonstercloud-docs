...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  const recaptchaV3Request = new RecaptchaV3ProxylessRequest({
    websiteURL: 'https://lessons.zennolab.com/captchas/recaptcha/v2_simple.php?level=high', // URL da página com captcha
...
recaptcha_v3_request = RecaptchaV3ProxylessRequest(
    websiteUrl="https://lessons.zennolab.com/captchas/recaptcha/v3.php?level=beta",  # URL da página com captcha
    websiteKey="6Le0xVgUAAAAAIt20XEB4rVhYOODgTl00d8juDob",  # Substitua pelo valor correto
    minScore=0.6,  
    pageAction="myverify"  
)
...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        Console.WriteLine("Solução do captcha: " + recaptchaV3Result.Solution.Value);
    }
}
```
