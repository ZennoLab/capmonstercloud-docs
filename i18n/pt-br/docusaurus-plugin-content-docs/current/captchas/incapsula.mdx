...
<Tabs className="full-width-tabs filled-tabs request-tabs" groupId="captcha-type">
  ...
  document.addEventListener('DOMContentLoaded', async () => {
      ...
      const cmcClient = CapMonsterCloudClientFactory.Create(
          new ClientOptions({ clientKey: 'YOUR_API_KEY' }) // Especifique sua chave de API do CapMonster Cloud
      );

      // Você pode verificar seu saldo se necessário
      const balance = await cmcClient.getBalance();
      console.log('Balance:', balance);

      // Configurando seu proxy personalizado
      const proxy = {
          proxyType: "http",
      ...
      const impervaRequest = new ImpervaRequest({
          websiteURL: 'https://example.com/', // URL da página protegida pelo Imperva
          metadata: {
              incapsulaScriptUrl: '_Incapsula_Resource?SWJIYLWA=719d34d31c8e3a6e6fffd425f7e032f3',
              incapsulaCookies:
...
API_KEY = "YOUR_API_KEY"  # Especifique sua chave de API do CapMonster Cloud
