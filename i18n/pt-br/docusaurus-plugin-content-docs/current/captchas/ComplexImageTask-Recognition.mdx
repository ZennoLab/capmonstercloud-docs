...
<TaskImageWrapper>
  ...
  <TaskImageBlock link="/docs/captchas/ComplexImageTask-Recognition#dli_ensemble">
    <TaskImage>![](./images/ComplexImageTask-Recognition/dli_ensemble.png)</TaskImage>
    <TaskImageText title="dli_ensemble" captchaId="complex-rec_oocl_rotate_new" />
  </TaskImageBlock>
...
>  IMPORTANTE: obtenha as imagens em base64 imediatamente antes de criar a tarefa para evitar erros durante a resolução (veja a seção [Como encontrar o Base64](#como-encontrar-o-base64)).
</span>
<br />
...
```
    ...
    <MethodDescription>
		**Solicitação**
		...
		```
		**Resposta:** "answer": [X, X, X, X, X, X, X, X, X], onde X é um valor inteiro de 1 a 4 para cada imagem.  
4 significa que a imagem não precisa ser girada;  
1–3 indicam o número de rotações da imagem no sentido anti-horário.
      ```json
      {
        "errorId":0,
        "status":"ready",
...
<TabItem value="proxyless" label="ComplexImageTask (sem proxy)" default className="method-panel-full">
    ...
    **Requisição**
    ...
    ```
    **Resposta:**  a resposta retorna os números extraídos do áudio.
    ```json
    {
      "solution": {
          "answer": [6, 8, 4, 1, 2, 3],
...
### Criar tarefa `dli_ensemble`

Na requisição, é necessário enviar **uma única imagem** (junto com o texto) no formato **base64**.
...
### Obter resultado da tarefa `dli_ensemble`
...
<TabItem value="proxyless" label="ComplexImageTask (sem proxy)" default className="method-panel-full">
  ...
  <MethodDescription>
    ...
    **Resposta:**  
    resultado do cálculo (**valor numérico**), retornado como **string**:
    ```json
    {
      "solution": {
        "answer": "1",
