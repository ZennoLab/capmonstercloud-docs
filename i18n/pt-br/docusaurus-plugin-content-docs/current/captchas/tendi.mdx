...
<Tabs className="full-width-tabs filled-tabs request-tabs" groupId="captcha-type">
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Opção com proxy (descomente se necessário)
    /*
    const tenDIRequest = new TenDIRequest({
      websiteURL: 'https://example.com',
...
ten_di_request = TenDiCustomTaskRequest(
    websiteUrl="https://example.com",   # URL da página com captcha
    websiteKey="189956587",             # Substitua pelo valor correto
...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        */
        ...
        Console.WriteLine("Dados da Solução do Captcha: " + string.Join(", ", tenDiRequestResult.Solution.Data));
        Console.WriteLine("Headers da Solução do Captcha: " + string.Join(", ", tenDiRequestResult.Solution.Headers));
    }
}
```
