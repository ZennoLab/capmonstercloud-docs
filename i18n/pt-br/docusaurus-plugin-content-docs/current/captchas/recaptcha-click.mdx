...
<TabItem value="proxyless" label="RecaptchaV2EnterpriseTaskProxyless (sem proxy)" default className="bordered-panel">
    ...
    **recaptcha**
    ...
    <ParamItem title="Grid (dentro dos metadados)" required type="string" />
    Tamanho da grade de imagem.
    Valores possíveis: 4x4, 3x3, 1x1

    ---

    <ParamItem title="TaskDefinition (dentro dos metadados)" required="required (se Task não estiver preenchido)" type="string" />
    ...
    <ParamItem title="Task (dentro dos metadados)" required="required (se TaskDefinition não estiver preenchido)" type="string" />
...
**Grid (dentro dos metadados)**: metadados adicionais relacionados aos tamanhos da grade de imagens.

**TaskDefinition (dentro dos metadados)**: identificador/tipo da descrição da tarefa, por exemplo: `/m/015qff` significa "Click on traffic lights".

![](./images/recaptcha-click/taskdefinition.png)

**Task (dentro dos metadados)**: metadados adicionais relacionados à tarefa.
