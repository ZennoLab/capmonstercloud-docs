...
Para isso, é preciso baixar o [arquivo da extensão](extension-main.mdx) e descompactá-lo. Na raiz do arquivo descompactado, encontra-se o arquivo `defaultSettings.json`, onde você pode alterar as configurações (a descrição dos parâmetros está disponível na [página](ext-settings.mdx)).
