...
Ao instalar a extensão via pacote (disponível para download [neste link para Chrome](https://zenno.link/chrome-actual-build) e [neste link para Firefox](https://zenno.link/firefox-actual-build)), é possível definir os parâmetros iniciais com os quais a extensão funcionará. Para isso, você precisa descompactar o pacote e editar o arquivo defaultSettings.json, e depois empacotá-lo novamente.
