...
<TabItem value="proxyless" label="RecaptchaV3TaskProxyless (without proxy)" default className="bordered-panel">
    ...
    **RecaptchaV3TaskProxyless**
    ...
    reCAPTCHA version. Supported values: `true` / `false` (also `1` / `0`, integer type). 
    
    Set `true` if you need to solve the reCAPTCHA as Enterprise — in this case, the task will be processed the same way as [RecaptchaV3EnterpriseTask](https://docs.capmonster.cloud/ru/docs/captchas/recaptcha-v3-enterprise-task/).
