...
>  IMPORTANT: retrieve the base64 images immediately before creating the task to avoid errors during solving (see the section [How to get base64](#how-to-get-base64)).
</span>
<br />
...
<TabItem value="proxyless" label="ComplexImageTask (without proxy)" default className="method-panel-full">
    ...
    ```json
    {
      "solution": {
        "answer": [
          { "X": 68.99999964, "Y": 201.954889 },
          { "X": 127.99999783999999, "Y": 281.54887104 },
          { "X": 181.00000776, "Y": 49.894734680000006 }
        ],
...
<TabItem value="proxyless" label="ComplexImageTask (proxyless)" default className="method-panel-full">
    ...
    **Request**
    ...
    ```
    **Response:** the response contains the digits extracted from the audio.
    ```json
    {
        "solution": {
            "answer": [6, 8, 4, 1, 2, 3],
...
### Create task `dli_ensemble`

In the request, you need to send **one image** (together with the text) in **base64** format.
...
<TabItem value="proxyless" label="ComplexImageTask (without proxy)" default className="method-panel-full">
  ...
  <MethodDescription>
    ...
    **Response:**  
    calculation result (**numeric value**), returned as a string:
    ```json
    {
      "solution": {
        "answer": "1",
