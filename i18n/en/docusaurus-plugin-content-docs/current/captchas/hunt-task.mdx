...
:::warning **Attention!**
When working with the target website, keep in mind that it may use special **cookies** to manage user sessions, perform anti‑bot checks, and coordinate requests. If the required cookies are missing or not sent with API requests, the server may reject the request, return an error, or require the verification process again.
:::
