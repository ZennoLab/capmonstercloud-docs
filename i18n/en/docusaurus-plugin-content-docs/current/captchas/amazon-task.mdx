...
:::warning **Warning!**
By default, CapMonster Cloud uses built-in proxies, so you don’t need to connect your own – their cost is already included in the price.  
Use your own proxies only in cases where the token is not accepted on the target site or access to built-in services is restricted. You can contact [support](https://helpdesk.zennolab.com/conversation/new) to clarify the details and check how well tokens are accepted on a specific site.  

If your proxy uses IP-based authorization, you must whitelist the address **116.203.55.208**.  
:::
