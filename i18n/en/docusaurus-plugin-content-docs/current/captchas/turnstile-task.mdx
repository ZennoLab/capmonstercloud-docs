...
You need to solve a **standard** Turnstile captcha, like [here](https://zenno.link/zl-support). Note that captchas on Cloudflare pages may look identical. See the end of the article to distinguish a standard Turnstile from a Cloudflare Challenge.

:::warning **Attention!**
* You **do not need** to use your own proxies for this task.
...
<TabItem value="proxyless" label="RecaptchaV2EnterpriseTaskProxyless (without proxy)" default className="bordered-panel">
  ...
  **token**
  ...
  Browser User-Agent.<br /> **Provide only the current Windows UA:** userAgentPlaceholder
