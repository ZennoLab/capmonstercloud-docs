...
> IMPORTANT: some parameter values are dynamic — they change with each render of the page using Altcha.  
> Extract them immediately before creating the task to avoid errors during solving.  
> See examples of parameter extraction in the section [How to find all required parameters for task creation](#how-to-find-all-required-parameters-for-task-creation).
</span>
<br />
...
<TabItem value="proxyless" label="CustomTask (without proxy)" default className="bordered-panel">
  ...
  **altcha**
  ...
  > **Important:** Always send the full value of the `salt` field!
...
Use the [getTaskResult](../api/methods/get-task-result.mdx) method to obtain the Altcha solution.
