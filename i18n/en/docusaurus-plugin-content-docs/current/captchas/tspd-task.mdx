...
If the server returns cookies in the following format (four lines):
...
If cookies are not returned or are returned in one of the following formats (usually three lines):
...
2. **Getting cookies.** Protection cookies are extracted from the response. If the cookie `TSPD_101_DID` is returned (which means the protection has already been passed) or the format is incorrect — execution stops.
...
5. **Final request.** The request is repeated with the received cookies, after which a normal HTML page is returned.
...
const API_KEY = "YOUR_API_KEY"; // Replace with your CapMonster API key
...
function extractValidTspdCookies(setCookies) {
  ...
  // There must be exactly 4 TS cookies
  ...
  try {
    ...
    const response = await gotScraping(url, {
      ...
      headers: {
        ...
        referer: "https://example.com/", // Replace with the actual referer
      },
    });
