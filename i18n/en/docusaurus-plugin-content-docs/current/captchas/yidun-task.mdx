...
<Tabs className="full-width-tabs">
  <TabItem value="jigsaw" label="Jigsaw" default>
    ...
    <TabItem value="jigsaw (slide_trajectory)" label="Jigsaw (slide_trajectory)" default>
    ![](./images/yidun-task/jigsaw-slide_trajectory.png)
  </TabItem>
...
>  IMPORTANT: some parameter values are dynamic — they change with each render of the page using Yidun.  
> Extract them immediately before creating the task to avoid errors during solving.  
> See examples of parameter extraction in the section [How to find all the required parameters](#how-to-find-all-the-required-parameters).
</span>
<br />
...
<TabItem value="proxy" label="YidunTask (with proxy)" className="bordered-panel">
  ...
  **Important**: must differ from the standard domains `c.dun.163.com` and `c-v6.dun.163.com`.  
  Specify only the domain, without the `https://` prefix. 
...
---
## How to find all the required parameters

`websiteURL` and `websiteKey`
