...
When installing the extension via a package (downloadable [at the link for Chrome](https://zenno.link/chrome-actual-build) and [at the link for Firefox](https://zenno.link/firefox-actual-build)), it is possible to set the initial parameters with which the extension will work. To do this, you need to unpack the package and edit the defaultSettings.json file, and then repackage it.
