...
1. Create a captcha task using [createTask](api/methods/create-task.mdx).
2. Wait for some time. Depending on system load, the response is usually received within **300** ms to **6** s.
3. Request the captcha solution using [getTaskResult](api/methods/get-task-result.mdx). If the captcha is not solved yet, go back to step 2.

Additional method:

* [Get](api/methods/get-balance.mdx) the current account balance.
